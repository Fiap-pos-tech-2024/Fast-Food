import { OrderInput, OrderStatus, OrderStatusType } from "../entities/types/orderTypes"
...
export default class Order {
    ...
    constructor(
        orderInput: OrderInput, itens: OrderItem[] | null = []) {
        ...
        calculateTotalValue() {
            this.value = this.itens?.reduce((total: number, item: OrderItem) => total + item.calculateTotalValue(), 0) ?? 0;
        }
}